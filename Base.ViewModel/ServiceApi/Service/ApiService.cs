...
namespace Base.ViewModel.ServiceApi.Service
{
    public class ApiService<T> : IApiService<T>
    {
        ...
        public ApiService(string apiBaseAddress)
        {
            ...
            {
                var client = new HttpClient(messageHandler) { BaseAddress = new Uri(apiBaseAddress) };
        ...
        public T GetApi(Priority priority)
        {
            switch (priority)
            {
                ...
                case Priority.UserInitiated: return UserInitiated;
                case Priority.Speculative: return Speculative;
